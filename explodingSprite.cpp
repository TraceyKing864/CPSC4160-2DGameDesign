...
ExplodingSprite::ExplodingSprite(Image* nImage, int nX, int nY) :
  Drawable("exploding", Vector2f(nX, nY), Vector2f(0.0, 0.0)),
  chunks(),
  freeList(),
  image(nImage){
    makeChunks(Gamedata::getInstance().getXmlInt("sprite_default/chunkSize"));
}
...
ExplodingSprite& ExplodingSprite::operator=(const ExplodingSprite& s) {
  Drawable::operator=(s);
  chunks = (s.chunks);
  freeList = (s.freeList);
  image = (s.image);
